<template>
  <div class="app-container">
    <el-card>
      <el-header style="font-size: 16px;margin-left:-25px;margin-top: -5px">用户订单</el-header>
      <el-form label-width="80px">
        <el-row :gutter="1">
          <el-col :span="6">
            <el-form-item label="商品名">
              <el-input v-model="searchMap.courseTitle" placeholder="商品名"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
      ...
      <el-table
        ...
        >
          <template slot-scope="scope">
            <img :src="scope.row.courseImg" style="width: 60px;height: 60px"/>
          </template>
        </el-table-column>
        <el-table-column
          align="center"
      ...
      <el-dialog :title="title" :visible.sync="dialogFormVisible" width="650px">
        <el-form :model="order" label-width="100px" >
          ...
          <el-input v-model="order.id" type="hidden" style="width: 400px"></el-input>
          <el-form-item label="商品订单号" >
            <el-input  disabled v-model="order.outTradeNo" type="text" style="width: 400px"></el-input>
          </el-form-item>
          <el-form-item label="商品名称" >
            <el-input disabled v-model="order.courseTitle" style="width: 400px"></el-input>
          </el-form-item>
          <el-form-item label="交易用户名" >
            <el-input disabled v-model="order.username" style="width: 400px"></el-input>
          </el-form-item>
...
<script>
  import {getOrderPageList,getOrderById,modifyOrder,delOrder} from '@/api/order'
  export default {
    name: "comment",
    data() {
      return {
    ...
    methods: {
      fetchData() {
        getOrderPageList(this.page,this.size,this.searchMap).then(response=>{
      ...
      //提交按钮
      submitBtn(){
        modifyOrder(this.order).then(response=>{
           if(response.success){
              this.$message({
                type:'success',
                message:'修改成功'
